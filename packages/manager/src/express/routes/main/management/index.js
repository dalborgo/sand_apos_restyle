...
function addRouters (router) {
  router.post('/management/import', fileUpload(), async function (req, res) {
    const { files = {} } = req, errors = [], idFieldsMap = {}
    ...
    const [controlRecordFunction, uniqueFields] = getControlRecord(firstColumns)
    console.log('uniqueFields:', uniqueFields)
    const onRecord = (record, { lines: line }) => {
      console.log('record:', record)
      
      const { checkedRecord, errors: rowsErrors } = controlRecordFunction(record, line, idFieldsMap)
